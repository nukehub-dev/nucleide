...
def test_chain_parse() -> None:
    chain = nucleide.read_chain(str(CHAIN))
...
def test_cram_decay_matches_analytic() -> None:
    ...
    dt = 1e5
    n0 = {"I135": 1e16}
    out = nucleide.deplete(chain, n0, dt, order=48)
...
def test_order16_and_48_agree() -> None:
    ...
    a = nucleide.deplete(chain, n0, 5e4, order=48)
    b = nucleide.deplete(chain, n0, 5e4, order=16)
    for k in a:
...
def test_bad_order_rejected() -> None:
    ...
    with pytest.raises(ValueError):
        nucleide.deplete(chain, {"I135": 1.0}, 1.0, order=32)


def test_unknown_rate_nuclide_rejected() -> None:
    chain = nucleide.read_chain(str(CHAIN))
    with pytest.raises(ValueError):
        nucleide.deplete(chain, {"I135": 1.0}, 1.0, rates={"Zzz9:(n,gamma)": 1e-3})
