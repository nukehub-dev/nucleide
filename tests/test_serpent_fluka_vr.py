...
class TestSerpent:
    def test_res(self) -> None:
        r = nucleide.read_serpent(str(FIX / "serpent" / "serp2_res.m"), "res")
    ...
    def test_dep(self) -> None:
        d = nucleide.read_serpent(str(FIX / "serpent" / "sample1_dep.m"), "dep")
        assert "ZAI" in d and "DAYS" in d
        assert len(d["ZAI"]) > 0

    def test_det(self) -> None:
        det = nucleide.read_serpent(str(FIX / "serpent" / "serp2_det.m"), "det")
        assert any("DET" in k for k in det)


class TestFluka:
    def test_usrbin_single(self) -> None:
        tallies = nucleide.read_usrbin(str(FIX / "fluka" / "fluka_usrbin_single.lis"))
    ...
    def test_usrbin_multiple(self) -> None:
        tallies = nucleide.read_usrbin(str(FIX / "fluka" / "fluka_usrbin_multiple.lis"))
        assert len(tallies) > 1

    def test_usrbin_degenerate(self) -> None:
        tallies = nucleide.read_usrbin(str(FIX / "fluka" / "fluka_usrbin_degenerate.lis"))
        assert len(tallies) >= 1


class TestMagicAndSampling:
    def setup_method(self) -> None:
        self.meshtal = nucleide.read_meshtal(
            str(FIX / "mcnp" / "meshtal" / "mcnp_meshtal_single_meshtal.txt")
        )
        self.tally = self.meshtal.tallies[4]

    def test_magic_total(self) -> None:
        out = nucleide.magic(self.tally)
    ...
    def test_sampler_modes(self) -> None:
        analog = nucleide.MeshSourceSampler(self.tally, "analog")
        uniform = nucleide.MeshSourceSampler(self.tally, "uniform")
    ...
    def test_user_mode(self) -> None:
        nve = self.tally.num_ves()
        sampler = nucleide.MeshSourceSampler(self.tally, "user", user_pdf=[1.0] * nve)
...
class TestWriters:
    def test_ssw_round_trip_bytes(self, tmp_path: Path) -> None:
        ...
        ssw = nucleide.read_ssw(str(src))
        tracks = ssw.tracks()
        out = tmp_path / "roundtrip.w"
        nucleide.write_ssw(ssw, str(out), tracks)
    ...
    def test_mesh_to_geom_structure(self) -> None:
        ...
        nve = 3 * 1 * 1
        mats = [("water", 1.0)] * nve
        deck = nucleide.mesh_to_geom(xb, yb, zb, mats, "test deck")
