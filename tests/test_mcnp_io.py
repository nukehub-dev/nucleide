...
class TestXsdir:
    def test_awr_and_tables(self) -> None:
        x = nucleide.mcnp.read_xsdir(str(FIXTURES / "xsdir" / "dummy_xsdir"))
    ...
    def test_nucs(self) -> None:
        x = nucleide.mcnp.read_xsdir(str(FIXTURES / "xsdir" / "dummy_xsdir"))
        assert x.nucs() == [nucleide.nuclei.Nuclide("H1").nucid]
...
class TestMeshtal:
    ...
    def test_multiple(self) -> None:
        m = nucleide.mcnp.read_meshtal(
            str(FIXTURES / "meshtal" / "mcnp_meshtal_multiple_meshtal.txt")
        )
...
class TestWwinp:
    ...
    def test_np(self) -> None:
        w = nucleide.mcnp.read_wwinp(str(FIXTURES / "wwinp" / "mcnp_wwinp_wwinp_np.txt"))
        assert w.ni == 2
        assert len(w.e) == 2
...
class TestSsw:
    def test_header_and_tracks(self) -> None:
        s = nucleide.mcnp.read_ssw(str(FIXTURES / "ssw" / "mcnp_surfsrc_onetrack.w"))
        assert s.kod == "mcnp"
        assert s.nrss == 1
        tracks = s.tracks()
        assert len(tracks) == 1
    ...
    def test_print_header(self) -> None:
        s = nucleide.mcnp.read_ssw(str(FIXTURES / "ssw" / "mcnp5_surfsrc.w"))
        txt = s.print_header()
